...
class StatisticsService:
    """Service xử lý logic tính toán thống kê"""
    ...
    def get_revenue_trend_data(self, year: int = None, start_date: datetime = None, end_date: datetime = None) -> Tuple[List[str], List[float]]:
        ...
        for item in data:
            ...
            if 0 <= month_idx < 12:
                revenues[month_idx] = item['revenue'] / 1000000  # Chuyển sang triệu đồng
    ...
    def get_duration_boxplot_data(self, start_date: datetime = None, end_date: datetime = None) -> Dict[str, List[int]]:
        ...
        if not data.get('single') and not data.get('monthly'):
            return {
                ...
                'Lượt / Vãng lai': [20, 35, 40, 50, 60, 30, 25, 45, 15, 20, 18, 25, 30, 22, 28]
            }
    ...
    def get_hour_histogram_data(self, start_date: datetime = None, end_date: datetime = None) -> List[int]:
        ...
        if not data or sum(data) == 0:
            # Dữ liệu mẫu
            return [0, 0, 0, 0, 0, 0, 1, 2, 3, 2, 1, 1, 1, 0, 1, 2, 3, 4, 3, 2, 1, 0, 0, 0]
